...
const comparativo = [
  ...
  {
    recurso: "Relatório de visitas e buscas",
    tradicional: false,
    vitrine: true,
  },
  { recurso: "Integração com Google", tradicional: false, vitrine: true },
  {
    recurso: "Promoções atualizadas periodicamente",
    tradicional: false,
    vitrine: true,
  },
];
        ...
        <Reveal>
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-5 text-lg leading-relaxed text-slate-500">
              Mais que um site institucional: uma solução completa de exposição
              de produtos, atendimento e captura de pedidos para o seu comércio
              físico.
            </p>
      ...
      <Section soft ariaLabelledby="pilares-title" className="pt-0 md:pt-0">
        ...
        <div className="flex flex-col gap-6">
          {pilares.map((pilar, index) => {
            ...
            return (
              <Reveal key={pilar.id} delay={index * 100}>
                <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-soft md:p-10">
            ...
            <table className="w-full text-left">
              ...
              <thead>
                <tr className="bg-bg-soft text-sm">
                  ...
                  <th
                    scope="col"
                    className="px-5 py-4 text-center font-semibold text-slate-500"
                  >
                    Site comum
                  </th>
                  <th
                    scope="col"
                    className="px-5 py-4 text-center font-semibold text-brand"
                  >
                    Vitrine Pro
                  </th>
                </tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {comparativo.map((linha) => (
                  <tr key={linha.recurso} className="text-[15px]">
                    <th
                      scope="row"
                      className="px-5 py-4 font-normal text-slate-700"
                    >
                      {linha.recurso}
                    </th>
      ...
      <Section soft ariaLabelledby="publico-title">
        <Reveal>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-lg text-slate-500">
              Para quem tem presença digital fraca ou inexistente e precisa
              vender mais sem contratar equipe de tecnologia ou marketing.
            </p>
          </div>
        </Reveal>
        <ul
        ...
        >
          {publicoAlvo.map((item, index) => (
            <Reveal key={item} delay={Math.min(index * 40, 300)}>
              <li>
  ...
  if (on) {
    return (
      <Check
        className={
          brand
            ? "mx-auto h-5 w-5 text-brand"
            : "mx-auto h-5 w-5 text-slate-400"
        }
        aria-label="Sim"
      />
    );
